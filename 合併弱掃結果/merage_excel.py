...
result["是否為新弱點(Y/N)"] = "N"
#result["OS版本"] = np.nan，然後插入到5欄位的後面
...
# #寫入excel分頁
# result_filter_Medium.to_excel(result_file, sheet_name= f'Medium-level({total_all_Medium_rows})')

# #過濾出Severity為Low
# # result_filter_Low = result[result['風險等級'].isin(["Low"])]
# # result_filter_Low.to_excel(result_file, sheet_name='(Low-level)')
...
# #寫入excel分頁
# result_filter_Low.to_excel(result_file, sheet_name= f'Low-level({total_all_Low_rows})')
